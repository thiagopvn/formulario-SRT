...
const generateFieldHTML = (field) => {
  ...
  switch (field.type) {
    ...
    case 'date':
      inputHTML = `
        <div class="floating-label">
          <input type="text" id="${field.key}" name="${field.key}" ${requiredAttr}
                 class="w-full px-4 py-3 bg-gray-100 dark:bg-gray-700 border-0 rounded-xl focus:ring-2 focus:ring-blue-500 transition-all date-input" 
                 placeholder=" "
                 data-date-field="true"
                 title="Formato: DD/MM/AAAA ou apenas AAAA">
          <label for="${field.key}">${field.label} ${requiredLabel}</label>
          <span class="text-xs text-gray-500 dark:text-gray-400 mt-1 block">Formato: DD/MM/AAAA ou apenas o ano</span>
        </div>
      `;
      break;
...
// Função para formatar entrada de data
const formatDateInput = (input) => {
  let value = input.value;
  let numericValue = value.replace(/\D/g, ''); // Remove não-dígitos
  
  // Se o usuário está apagando, não formatar
  if (input.value.length < input.dataset.previousLength) {
    input.dataset.previousLength = input.value.length;
    return;
  }
  
  // Se tem 4 dígitos e nenhuma barra, assumir que é apenas o ano
  if (numericValue.length === 4 && !value.includes('/')) {
    input.dataset.previousLength = input.value.length;
    return;
  }
  
  // Formatação automática apenas se o usuário estiver digitando uma data completa
  if (numericValue.length > 4 || value.includes('/')) {
    if (numericValue.length <= 2) {
      input.value = numericValue;
    } else if (numericValue.length <= 4) {
      input.value = numericValue.slice(0, 2) + '/' + numericValue.slice(2);
    } else if (numericValue.length <= 8) {
      input.value = numericValue.slice(0, 2) + '/' + numericValue.slice(2, 4) + '/' + numericValue.slice(4, 8);
    } else {
      input.value = numericValue.slice(0, 2) + '/' + numericValue.slice(2, 4) + '/' + numericValue.slice(4, 8);
    }
  }
...
// Função para validar data flexível
const validateFlexibleDate = (input) => {
  ...
  if (/^\d{4}$/.test(value)) {
    ...
    if (year >= 1900 && year <= new Date().getFullYear()) {
      ...
      input.setCustomValidity('Ano inválido. Use um ano entre 1900 e o ano atual.');
      return false;
    }
  }
  ...
  // Formato inválido
  input.setCustomValidity('Use o formato DD/MM/AAAA ou apenas AAAA.');
  return false;
};
...
const setupEventListeners = () => {
  ...
  document.addEventListener('input', (e) => {
    ...
    if (e.target.dataset.dateField === 'true') {
...
const validateCurrentStep = () => {
  ...
  const allDateFields = currentSection.querySelectorAll('[data-date-field="true"]');
  let isValid = true;
  ...
  requiredFields.forEach(field => {
    ...
    // Validação especial para campos de data obrigatórios
    if (field.dataset.dateField === 'true') {
      if (!validateFlexibleDate(field)) {
        field.classList.add('animate-shake');
        if (!firstInvalidField) firstInvalidField = field;
        isValid = false;
  ...
  // Validar todos os campos de data (mesmo os não obrigatórios)
  ...
  if (!isValid) {
    if (firstInvalidField) {
      ...
      if (firstInvalidField.validationMessage) {
        showToast(firstInvalidField.validationMessage, 'error');
      } else {
