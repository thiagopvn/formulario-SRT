...
const submitForm = async (event) => {
  ...
  try {
    ...
    if (typeof houseData.vinculoEmpregaticio === 'string') {
      houseData.vinculoEmpregaticio = [houseData.vinculoEmpregaticio];
    }
...
const generateEmailReport = (data) => {
    ...
    <body>
      <div class="container">
        <div class="header">
          ...
          <p>Relatório Completo do Formulário</p>
  ...
  // SEÇÃO 1: INFORMAÇÕES DO MUNICÍPIO
  html += `
          <div class="section">
            ...
            <table class="data-table">
              <tr>
                ...
                <td>${formatValue(data.regiaoSaude)}</td>
              </tr>
              <tr>
                <td>Município</td>
                <td>${formatValue(data.municipio)}</td>
              </tr>
              <tr>
                <td>Coordenação de Saúde Mental</td>
                <td>${formatValue(data.coordenacaoSaudeMental)}</td>
              </tr>
              <tr>
                <td>Responsável pelo Preenchimento</td>
                <td>${formatValue(data.responsavelPreenchimento)}</td>
              </tr>
              <tr>
                <td>Telefone do Responsável</td>
                <td>${formatValue(data.telefoneResponsavelPreenchimento)}</td>
              </tr>
              <tr>
                <td>E-mail do Responsável</td>
                <td>${formatValue(data.emailResponsavelPreenchimento)}</td>
              </tr>
              <tr>
                <td>Data do Preenchimento</td>
                <td>${formatDate(data.dataPreenchimentoMunicipio)}</td>
              </tr>
              <tr>
                <td>CAPS de Referência</td>
                <td>${formatValue(data.capsVinculadaSRT)}</td>
              </tr>
              <tr>
                <td>CNES do CAPS</td>
                <td>${formatValue(data.cnesCapsVinculada)}</td>
              </tr>
            </table>
          </div>`;

  // SEÇÃO 2: DADOS DA SRT
  html += `
          <div class="section">
            <div class="section-header">
            ...
            <table class="data-table">
              <tr>
                <td>Data do Preenchimento</td>
                <td>${formatDate(data.dataPreenchimento)}</td>
              </tr>
              <tr>
                <td>Nome da Residência</td>
                <td><span class="highlight">${formatValue(data.nomeResidencia)}</span></td>
              </tr>
              <tr>
                <td>Responsável pela Residência</td>
                <td>${formatValue(data.responsavelNome)}</td>
              </tr>
              <tr>
                <td>Cargo/Função</td>
                <td>${formatValue(data.responsavelCargo)}</td>
              </tr>
              <tr>
                <td>Telefone de Contato</td>
                <td>${formatValue(data.contatoResponsavel)}</td>
              </tr>
              <tr>
                <td>Nome do CAPS</td>
                <td>${formatValue(data.nomeCaps)}</td>
              </tr>
              <tr>
                <td>CNES do CAPS</td>
                <td>${formatValue(data.cnesCaps)}</td>
              </tr>
              <tr>
                <td>Tipo de SRT</td>
                <td><span class="highlight">${formatValue(data.tipoSRT)}</span></td>
              </tr>
              <tr>
                <td>Esfera de Gestão</td>
                <td>${formatValue(data.esferaGestao)}</td>
              </tr>
              <tr>
                <td>Situação da Habilitação</td>
                <td>${formatValue(data.situacaoHabilitacao)}</td>
              </tr>
              ${data.numeroPortaria ? `
              <tr>
                <td>Número da Portaria</td>
                <td>${formatValue(data.numeroPortaria)}</td>
              </tr>` : ''}
              ${data.dataPortaria ? `
              <tr>
                <td>Data da Portaria</td>
                <td>${formatDate(data.dataPortaria)}</td>
              </tr>` : ''}
              <tr>
                <td>Data de Inauguração</td>
                <td>${formatDate(data.dataInauguracao)}</td>
              </tr>
            </table>
          </div>`;

  // SEÇÃO 3: DADOS DA RESIDÊNCIA
  html += `
          <div class="section">
            <div class="section-header">
              <div class="section-icon">🏠</div>
              <h2>Dados da Residência Terapêutica</h2>
            </div>
            <table class="data-table">
              <tr>
                ...
                <td>
                  ...
                  CEP: ${formatValue(data.cep)}
                </td>
              </tr>
              <tr>
                <td>Localização</td>
                <td>${formatValue(data.localizacao)}</td>
              </tr>
              <tr>
                <td>Quantidade de Quartos</td>
                <td>${formatValue(data.quartos)}</td>
              </tr>
              <tr>
                <td>Quantidade de Salas</td>
                <td>${formatValue(data.salas)}</td>
              </tr>
              <tr>
                <td>Quantidade de Cozinhas</td>
                <td>${formatValue(data.cozinhas)}</td>
              </tr>
              <tr>
                <td>Quantidade de Banheiros</td>
                <td>${formatValue(data.banheiros)}</td>
              </tr>
              <tr>
                <td>Quantidade de Varandas</td>
                <td>${formatValue(data.varanda)}</td>
              </tr>
              <tr>
                <td>Quantidade de Lavanderias</td>
                <td>${formatValue(data.lavanderia)}</td>
              </tr>
              <tr>
                <td>Quantidade de Despensas</td>
                <td>${formatValue(data.despensa)}</td>
              </tr>
              ${data.outros ? `
              <tr>
                <td>Outros Cômodos</td>
                <td>${formatValue(data.outros)}</td>
              </tr>` : ''}
            </table>
          </div>`;

  // SEÇÃO 4: DADOS GERAIS DOS MORADORES
  html += `
          <div class="section">
            <div class="section-header">
              <div class="section-icon">👥</div>
              <h2>Dados Gerais dos Moradores</h2>
            </div>
            <table class="data-table">
              <tr>
                <td>Total de Moradores na Implantação</td>
                <td>${formatValue(data.totalMoradores)}</td>
              </tr>
              <tr>
                <td>Moradores Cadastrados no CNES</td>
                <td>${formatValue(data.vagasTotais)}</td>
              </tr>
              <tr>
                <td>Moradores Atuais</td>
                <td><span class="highlight">${formatValue(data.vagasOcupadas)}</span></td>
              </tr>
              <tr>
                <td>Vagas Disponíveis</td>
                <td>${formatValue(data.vagasDisponiveis)}</td>
              </tr>
            </table>
          </div>`;

  // SEÇÃO 5: DADOS DA EQUIPE/CUIDADORES
  html += `
          <div class="section">
            <div class="section-header">
              <div class="section-icon">👨‍⚕️</div>
              <h2>Dados da Equipe/Cuidadores</h2>
            </div>
            <table class="data-table">
              <tr>
                <td>Total de Profissionais</td>
                <td><span class="highlight">${formatValue(data.totalProfissionais)}</span></td>
              </tr>
              <tr>
                <td>Número de Cuidadores</td>
                <td>${formatValue(data.totalCuidadores)}</td>
              </tr>
              <tr>
                <td>Técnicos de Enfermagem</td>
                <td>${formatValue(data.totalTecnicos)}</td>
              </tr>
              <tr>
                <td>Enfermeiros</td>
                <td>${formatValue(data.totalEnfermeiros)}</td>
              </tr>
              <tr>
                <td>Outros Profissionais</td>
                <td>${formatValue(data.totalOutros)}</td>
              </tr>
              <tr>
                <td>Escala de Trabalho</td>
                <td>${formatValue(data.escalaTrabalho)}</td>
              </tr>
              <tr>
                <td>Proporção Cuidador/Morador</td>
                <td>${formatValue(data.relacaoCuidadorMorador)}</td>
              </tr>
              <tr>
                <td>Cuidadores por Turno</td>
                <td>${formatValue(data.cuidadoresPorTurno)}</td>
              </tr>
              <tr>
                <td>Participa de Educação Permanente?</td>
                <td>${formatValue(data.participaEducacao)}</td>
              </tr>
              ${data.quemPromoveEducacao ? `
              <tr>
                <td>Detalhes da Educação Permanente</td>
                <td>${formatValue(data.quemPromoveEducacao)}</td>
              </tr>` : ''}
              <tr>
                <td>Realiza Reuniões Regulares?</td>
                <td>${formatValue(data.reunioesRegulares)}</td>
              </tr>
              <tr>
                <td>Vínculos Empregatícios</td>
                <td>${formatArray(data.vinculoEmpregaticio)}</td>
              </tr>
            </table>
          </div>`;

  // SEÇÃO 6: DADOS INDIVIDUAIS DOS MORADORES
  if (data.residents && data.residents.length > 0) {
    html += `
          <div class="section">
            <div class="section-header">
              <div class="section-icon">🏥</div>
              <h2>Dados Individuais dos Moradores (${data.residents.length})</h2>
            </div>
      ...
      html += `
              <div class="resident-card">
                <div class="resident-header">
                  ...
                  <h3 style="margin: 0; font-size: 18px; color: #0284c7;">
                ...
                <table class="data-table">
                  <tr>
                    ...
                    <td>${formatDate(resident.dataNascimento)}</td>
                  </tr>
                  <tr>
                    <td>Idade</td>
                    <td>${formatValue(resident.idade)} anos</td>
                  </tr>
                  <tr>
                    <td>Instituição de Origem</td>
                    <td>${formatValue(resident.instituicaoOrigem)}</td>
                  </tr>
                  ${resident.cnesOrigem ? `
                  <tr>
                    <td>CNES da Instituição de Origem</td>
                    <td>${formatValue(resident.cnesOrigem)}</td>
                  </tr>` : ''}
                  <tr>
                    <td>Tempo de Internação Anterior</td>
                    <td>${formatValue(resident.tempoInternacao)} anos</td>
                  </tr>
                  <tr>
                    <td>Raça/Cor</td>
                    <td>${formatValue(resident.racaCor)}</td>
                  </tr>
                  <tr>
                    <td>Sexo Biológico</td>
                    <td>${formatValue(resident.generoNascimento)}</td>
                  </tr>
                  <tr>
                    <td>Identidade de Gênero</td>
                    <td>${formatValue(resident.identidadeGenero)}</td>
                  </tr>
                  <tr>
                    <td>Município de Origem</td>
                    <td>${formatValue(resident.origemTerritorial)}</td>
                  </tr>
                  ${resident.vinculoMunicipio ? `
                  <tr>
                    <td>Vínculo com Município Atual</td>
                    <td>${formatValue(resident.vinculoMunicipio)}</td>
                  </tr>` : ''}
                  <tr>
                    <td>Beneficiário do PVC?</td>
                    <td>${formatValue(resident.participaPVC)}</td>
                  </tr>
                  <tr>
                    <td>Mantém Contato Familiar?</td>
                    <td>${formatValue(resident.vinculoFamiliar)}</td>
                  </tr>
                  ${resident.descricaoVinculo ? `
                  <tr>
                    <td>Descrição do Vínculo Familiar</td>
                    <td>${formatValue(resident.descricaoVinculo)}</td>
                  </tr>` : ''}
                  <tr>
                    <td>Frequência ao CAPS</td>
                    <td>${formatValue(resident.frequenciaCaps)}</td>
                  </tr>
                  <tr>
                    <td>Acompanhamento na UBS</td>
                    <td>${formatValue(resident.frequenciaUBS)}</td>
                  </tr>
                  <tr>
                    <td>Frequenta Instituição de Ensino?</td>
                    <td>${formatValue(resident.escola)}</td>
                  </tr>
                  ${resident.qualEscola ? `
                  <tr>
                    <td>Qual Instituição/Curso?</td>
                    <td>${formatValue(resident.qualEscola)}</td>
                  </tr>` : ''}
                  <tr>
                    <td>Acompanhamento CRAS/CREAS</td>
                    <td>${formatValue(resident.crasCreas)}</td>
                  </tr>
                  <tr>
                    <td>Benefícios</td>
                    <td>${formatArray(resident.beneficios)}</td>
                  </tr>
                  ${resident.comorbidades && resident.comorbidades.length > 0 ? `
                  <tr>
                    <td>Comorbidades</td>
                    <td>${formatArray(resident.comorbidades)}</td>
                  </tr>` : ''}
                </table>
              </div>`;
    });
  ...
  html += `
        ...
        <div class="footer">
          ...
          <p>Mantenha este e-mail arquivado para seus registros</p>
